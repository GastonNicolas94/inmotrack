...
async function getPeriodosContrato(
  _req: NextRequest,
  ...
  try {
    ...
    const respuesta = periodos.map((p) => {
      ...
      const montoTotal = p.cargos.reduce(
        (acc, c) => acc.plus(new Decimal(c.monto)),
        new Decimal(0)
      );
...
export const GET = withObservability(getPeriodosContrato);
