    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (data.id_contrato) {
        ...
        if (periodoAbierto) {
          ...
          // Si el gasto es a cargo del inquilino, no puede quedar esperando
          // al cierre del período: si ya tiene crédito disponible de un
          // sobrante viejo, se consume ahora mismo, en el momento en que
          // nace la deuda. pct_comision/es_propia no aplican acá — un
          // GASTO nunca genera comisión (ver aplicarCreditoDisponible).
