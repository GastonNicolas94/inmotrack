...
type ContratoAjustable = Pick<
  Contrato,
  "id" | "indice_act" | "monto_base"
>;

export const AjustesContratoService = {
  async listarPorContrato(idContrato: number) {
    return prisma.ajusteContrato.findMany({
  ...
  async obtenerPendiente(idContrato: number) {
    return prisma.ajusteContrato.findFirst({
  ...
  async aplicar(
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (ajuste.estado !== "PENDIENTE") {
        throw new Error("El ajuste ya fue aplicado.");
      }
      ...
      if (!yaPendiente) {
        await tx.outboxCierrePeriodo.create({
          data: { id_contrato: idContrato, estado: "PENDIENTE" },
        });
      }
